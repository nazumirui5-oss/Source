...
local function ApplyTrip(state)
    ...
    -- >>> LOGIKA ANTI-BOMB: Matikan deteksi sentuhan (CanTouch) saat trip <<<
    for _, part in ipairs(char:GetDescendants()) do
        if part:IsA("BasePart") then
            -- Jika state = true (sedang trip), CanTouch diatur ke false agar bom musuh tidak mendeteksi sentuhan
    ...
    if state then
        -- Matikan kemampuan berdiri otomatis agar karakter tetap tergeletak selama toggle aktif
        hum:SetStateEnabled(Enum.HumanoidStateType.GettingUp, false)
        
        -- Ubah state ke FallingDown (indeks 0, metode trip asli Infinite Yield)
        hum:ChangeState(Enum.HumanoidStateType.FallingDown)
        
        -- Miringkan sudut karakter 30 derajat ke depan agar jatuh tersungkur secara instan
        pcall(function()
...
-- INTEGRASI FITUR TRIP FALL DI TAB MOVEMENT (MENGGANTIKAN RAGDOLL)
